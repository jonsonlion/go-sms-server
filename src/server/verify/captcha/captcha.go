...
func NewLen(length int) (id string) {
	id = randomId()
	globalStore.Set(id, captcha.RandomDigits(length))
	return
}

func NewLenCustom(length int,id string) (string) {
	globalStore.Set(id, captcha.RandomDigits(length))
	return id
}
...
func Reload(id string) bool {
	...
	globalStore.Set(id, captcha.RandomDigits(len(old)))
	return true
}
...
func WriteAudio(w io.Writer, id string, lang string) error {
	...
	_, err := captcha.NewAudio(id, d, lang).WriteTo(w)
	return err
}
